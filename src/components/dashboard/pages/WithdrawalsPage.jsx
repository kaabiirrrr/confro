...
const FEE_RATE = 0.03;
const inputCls = "w-full bg-slate-900/5 dark:bg-white/[0.03] border border-slate-900/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-950 dark:text-white text-sm placeholder-slate-900/20 dark:placeholder-white/20 focus:outline-none focus:border-accent transition-all duration-200 backdrop-blur-md";
...
function BankFormFields({ selectedBank, details, setDetail, handleBankSelect, errors }) {
  ...
  const Label = ({ children }) => <label className="block text-slate-900/30 dark:text-white/30 text-[10px] font-bold uppercase tracking-widest mb-2">{children}</label>;
  return (
    <div className="space-y-4">
      <div>
        <Label>Bank name</Label>
        <CustomDropdown 
          options={INDIAN_BANKS.map(b => ({ label: b.name, value: b.name, description: `${b.acLen} digits` }))} 
          value={selectedBank?.name} 
          onChange={(val) => handleBankSelect(INDIAN_BANKS.find(b => b.name === val))} 
...
function AddBankModal({ onClose, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-24 px-4 pb-4 backdrop-blur-md bg-black/40" onClick={onClose}>
      <motion.div onClick={e => e.stopPropagation()}
        initial={{ opacity: 0, scale: 0.95, y: -20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -20 }}
        transition={{ type: 'spring', damping: 25, stiffness: 300 }}
        className="bg-white dark:bg-secondary/95 border border-slate-900/10 dark:border-white/10 rounded-xl w-full max-w-[440px] shadow-2xl overflow-hidden backdrop-blur-2xl flex flex-col">
        <div className="flex items-start justify-between px-8 pt-8 pb-4">
          <div>
            <h2 className="text-slate-950 dark:text-white text-xl font-bold tracking-tight">Add Bank Account</h2>
            <p className="text-slate-900/40 dark:text-white/40 text-xs mt-1 font-medium">Link a verified account for fast withdrawals</p>
          </div>
          <button onClick={onClose} className="p-2 text-white/20 hover:text-accent transition-colors"><X size={20} /></button>
        </div>
        <form onSubmit={handleSubmit} className="px-8 pb-8 space-y-6 overflow-y-auto">
          ...
          <div className="flex items-start gap-3 p-3 bg-accent/5 border border-accent/10 rounded-xl">
            ...
            <p className="text-slate-900/40 dark:text-white/40 text-[10px] leading-relaxed">Account details are encrypted. Ensure the holder name matches your profile for faster approval.</p>
          </div>
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose}
              className="flex-1 py-3.5 rounded-full border border-slate-900/10 dark:border-white/10 text-slate-900/40 dark:text-white/40 hover:text-slate-950 dark:hover:text-white text-xs font-bold uppercase tracking-widest transition-all">
              Cancel
            </button>
...
function WithdrawalModal({ available, onClose, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-start justify-center pt-24 px-4 pb-4 backdrop-blur-md bg-black/40" onClick={onClose}>
      <motion.div onClick={e => e.stopPropagation()}
        initial={{ opacity: 0, scale: 0.95, y: -20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -20 }}
        transition={{ type: 'spring', damping: 25, stiffness: 300 }}
        className="bg-white dark:bg-secondary/95 border border-slate-900/10 dark:border-white/10 rounded-xl w-full max-w-[460px] shadow-2xl overflow-hidden backdrop-blur-2xl flex flex-col">
        
        <div className="flex items-start justify-between px-8 pt-8 pb-4">
          <div>
            <h2 className="text-slate-950 dark:text-white text-xl font-bold tracking-tight">Withdraw Funds</h2>
            <p className="text-slate-900/40 dark:text-white/40 text-[10px] uppercase font-bold tracking-widest mt-1">Available: <span className="text-accent">{fmtINR(availableINR)}</span></p>
          </div>
          <button onClick={onClose} className="p-2 text-white/20 hover:text-accent transition-colors"><X size={20} /></button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-8 pb-8 space-y-6 overflow-y-auto">
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-900/20 dark:text-white/20 text-lg group-focus-within:text-accent group-focus-within:font-bold transition-all">₹</span>
              <input type="number" min={500} step="1" value={amount}
                onChange={e => { setAmount(e.target.value); setErrors(p => ({ ...p, amount: undefined })); }}
                placeholder="0.00"
                className="w-full bg-slate-900/5 dark:bg-white/[0.03] border border-slate-900/10 dark:border-white/10 rounded-2xl pl-10 pr-20 py-4 text-slate-950 dark:text-white text-xl font-bold placeholder-slate-900/5 dark:placeholder-white/5 focus:outline-none focus:border-accent/40 focus:bg-slate-900/[0.08] dark:focus:bg-white/[0.05] transition-all" />
              <button type="button"
                onClick={() => { setAmount(Math.floor(availableINR).toString()); setErrors(p => ({ ...p, amount: undefined })); }}
                className="absolute right-3 top-1/2 -translate-y-1/2 px-3 py-1.5 rounded-xl bg-slate-900/5 dark:bg-white/5 hover:bg-slate-900/10 dark:hover:bg-white/10 text-slate-900/40 dark:text-white/40 hover:text-slate-950 dark:hover:text-white text-[10px] font-bold uppercase tracking-widest border border-slate-900/10 dark:border-white/10 transition-all">
                Max Funds
              </button>
            </div>
            <Err field="amount" />
          ...
          {amtNum >= 500 && (
            <div className="flex items-center justify-between p-4 bg-accent/[0.03] rounded-2xl border border-accent/10">
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-900/30 dark:text-white/30">Fee (3%) <span className="text-red-400 opacity-50 ml-1">−₹{fee.toLocaleString()}</span></span>
              <span className="text-sm font-bold text-slate-950 dark:text-white tracking-tight">You get <span className="text-accent">₹{youReceive.toLocaleString()}</span></span>
            </div>
          )}
          ...
          <div className="space-y-3">
            ...
            <div className="flex p-1 bg-slate-900/[0.03] dark:bg-white/[0.03] border border-slate-900/10 dark:border-white/10 rounded-2xl">
          ...
          <div className="flex items-center gap-2 text-slate-900/20 dark:text-white/20 text-[10px] uppercase font-bold tracking-widest bg-slate-900/[0.01] dark:bg-white/[0.01] p-3 rounded-xl border border-slate-900/5 dark:border-white/5">
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose}
              className="flex-1 py-4 rounded-full border border-slate-900/10 dark:border-white/10 text-slate-900/40 dark:text-white/40 hover:text-slate-950 dark:hover:text-white text-[10px] font-bold uppercase tracking-widest transition-all">
              Cancel
            </button>
            <button type="submit" disabled={submitting}
          ...
          <div className="rounded-2xl bg-slate-900/[0.02] dark:bg-transparent border border-slate-900/10 dark:border-white/10 p-6 flex flex-col justify-between min-h-[140px] relative overflow-hidden group backdrop-blur-xl">
            ...
            <div className="flex items-start justify-between gap-4 relative z-10">
              <div>
                ...
                <p className="text-slate-950 dark:text-white font-bold tracking-tighter leading-none" style={{ fontSize: '2.5rem' }}>{fmtINR(availableINR)}</p>
              </div>
              {canWithdraw && (
                <button onClick={() => setShowModal(true)}
                  className="px-5 py-2.5 rounded-full bg-slate-900/5 dark:bg-white/5 hover:bg-slate-900/10 dark:hover:bg-white/10 text-slate-950 dark:text-white text-xs font-bold transition-all border border-slate-900/10 dark:border-white/10 backdrop-blur-md">
                  Withdraw
                </button>
              )}
            </div>
            <div className="flex items-center justify-between gap-4 pt-4 border-t border-slate-900/5 dark:border-white/5 relative z-10">
              <p className="text-slate-900/40 dark:text-white/40 text-[10px] font-medium uppercase tracking-wider truncate">Last Payout: <span className="text-slate-900/60 dark:text-white/60">{lastWithdrawal ? fmtDate(lastWithdrawal.created_at) : 'Never'}</span></p>
              {!canWithdraw && <p className="text-red-400/60 text-[10px] font-bold uppercase tracking-wider italic shrink-0">Need ₹{Math.ceil(500 - availableINR)} more</p>}
            </div>
          </div>

          {/* Three stats in one line for mobile */}
          <div className="grid grid-cols-3 lg:grid-cols-3 gap-3 sm:gap-5">
            {[
              { label: 'Earned',    value: parseFloat(balance?.total_earned    || 0) * USD_TO_INR },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8">
        ...
        <div>
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6 mb-6">
            ...
            <div className="flex items-center justify-between sm:justify-start gap-4 sm:gap-8 border-b border-slate-900/10 dark:border-white/10 w-full sm:w-auto overflow-x-auto no-scrollbar">
              {STATUS_FILTERS.map(({ key, label }) => (
                <button key={key} onClick={() => setStatusFilter(key)}
                  className={`relative pb-4 px-1 sm:px-2 text-[10px] uppercase font-black tracking-widest transition-all shrink-0 ${statusFilter === key ? 'text-accent' : 'text-slate-900/30 dark:text-white/30 hover:text-slate-950 dark:hover:text-white'}`}>
                  {label}
                  {statusFilter === key && (
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 rounded-3xl border border-dashed border-slate-900/10 dark:border-white/5 bg-transparent text-center backdrop-blur-sm">
              ...
              <p className="text-slate-900/60 dark:text-white/60 text-sm font-bold tracking-tight mb-1">No withdrawals yet</p>
              <p className="text-slate-900/20 dark:text-white/20 text-xs mb-6 max-w-[240px]">Start by withdrawing your earnings to your bank account.</p>
              <button onClick={() => setShowModal(true)} disabled={!canWithdraw}
                className="px-6 py-3 rounded-full bg-accent hover:bg-accent/90 disabled:opacity-30 text-white text-xs font-bold transition-all">
                Withdraw Funds
              </button>
            </div>
          ) : (
            <div className="rounded-2xl border border-slate-900/10 dark:border-white/10 bg-transparent overflow-hidden backdrop-blur-xl">
              ...
              <div className="divide-y divide-slate-900/5 dark:divide-white/5">
                {filtered.map(w => {
                  ...
                  return (
                    <div key={w.id} className="flex flex-col sm:grid sm:grid-cols-[1fr_120px_120px_120px_64px] gap-4 px-6 py-5 hover:bg-slate-900/[0.02] dark:hover:bg-white/[0.02] transition-colors items-center sm:items-center group border-b border-slate-900/5 dark:border-white/5 last:border-0">
                      <div className="w-full sm:w-auto flex justify-between sm:block">
                        <div>
                          <p className="text-slate-900/80 dark:text-white/80 text-sm font-medium">{fmtDate(w.created_at)}</p>
                          <p className="text-slate-900/10 dark:text-white/10 text-[10px] mt-1 font-mono group-hover:text-slate-900/20 dark:group-hover:text-white/20 transition-colors uppercase tracking-tight">{w.id?.slice(0, 12)}</p>
                        </div>
                        <div className="sm:hidden text-right">
                           <p className="text-slate-950 dark:text-white font-bold text-sm tracking-tight">{fmtINR(w.amount)}</p>
                           <p className="text-slate-900/40 dark:text-white/40 text-[10px] uppercase font-bold tracking-widest mt-0.5">{w.method?.replace('_', ' ') || '—'}</p>
                        </div>
                      </div>
                      <p className="hidden sm:block text-slate-950 dark:text-white font-bold text-sm text-right tracking-tight">{fmtINR(w.amount)}</p>
            ...
            {bankAccounts.length === 0 ? (
              <div className="px-6 py-10 text-center">
                <div className="w-16 h-16 flex items-center justify-center mx-auto mb-4">
                  <img src="/Icons/icons8-bank-96.png" alt="Bank" className="w-12 h-12 object-contain" />
                </div>
                <p className="text-slate-900/30 dark:text-white/30 text-xs mb-4">No bank accounts saved yet</p>
          ...
          <div className="rounded-2xl border border-slate-900/10 dark:border-white/10 bg-transparent p-6 space-y-4 backdrop-blur-xl">
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={label} className="flex items-center justify-between border-b border-slate-900/5 dark:border-white/5 pb-2 last:border-0 last:pb-0">
                  <span className="text-slate-900/30 dark:text-white/30 text-[10px] font-bold uppercase tracking-widest">{label}</span>
                  <span className="text-slate-900/70 dark:text-white/70 text-xs font-semibold">{value}</span>
                </div>
              ))}
            </div>
          </div>
