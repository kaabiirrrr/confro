      ...
      {loading ? (
        ...
        <>
          <div className="bg-transparent border border-white/10 rounded-2xl overflow-hidden mb-4">
            {/* Header */}
            <div className="grid grid-cols-[140px_1fr_1fr_100px_120px] gap-4 px-8 py-5 border-b border-white/10 text-[9px] text-white/20 font-black uppercase tracking-widest">
              <span>Date</span>
              <span>Contract</span>
            ...
            {rows.map((tx, i) => {
              ...
              return (
                <div key={tx.id ?? i} className="grid grid-cols-[140px_1fr_1fr_100px_120px] gap-4 px-8 py-5 border-b border-white/5 last:border-0 hover:bg-white/5 transition items-center group">
                  <span className="text-white/50 text-sm font-medium">{fmtDate(tx.created_at ?? tx.date)}</span>

                  <span className="text-white text-sm font-bold truncate block group-hover:text-accent transition-colors">{tx.contract_title || tx.contract?.title || `Contract #${tx.contract_id}`}</span>
