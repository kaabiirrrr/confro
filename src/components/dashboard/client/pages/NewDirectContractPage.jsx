...
const inputCls = "w-full bg-secondary border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent/40 transition-all placeholder:text-white/20";
...
export default function NewDirectContractPage() {
  ...
  return (
    <div className="max-w-[1630px] mx-auto px-4 sm:px-6 lg:px-8 mt-6 pb-12 text-white">
      {/* Back */}
      <button
      ...
      <div className="mb-8">
        <h1 className="text-2xl font-semibold text-white">New Direct Contract</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        {/* Freelancer */}
        <div>
          <label className="block text-[10px] text-white/30 uppercase font-bold tracking-wider mb-2">Freelancer</label>
          {prefillId && loadingFreelancer ? (
            ...
            <div className="flex items-center gap-3 bg-secondary border border-white/10 rounded-2xl px-4 py-4">
              {freelancerAvatar ? (
              ...
              <button
                ...
                className="text-white/30 hover:text-white text-[10px] uppercase font-bold tracking-wider transition"
              >
                Change
              </button>
            </div>
            ...
            <div>
              <input
                ...
                className={inputCls}
              />
              {errors.freelancer_id && <p className="text-red-400 text-xs mt-1">{errors.freelancer_id}</p>}
        ...
        <div className="space-y-2">
          <label className="block text-[10px] text-white/30 uppercase font-bold tracking-wider">Project Type</label>
        ...
        <div className="flex gap-4 pt-4 border-t border-white/5">
          <button
            type="button"
            onClick={() => navigate('/client/direct-contracts')}
            className="flex-1 py-3 rounded-xl border border-white/10 text-white/50 hover:text-white hover:bg-white/5 text-sm font-medium transition"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={submitting}
            className="flex-1 py-3 rounded-xl bg-accent text-white text-sm font-medium hover:bg-accent/90 transition shadow-lg shadow-accent/20 disabled:opacity-50 flex items-center justify-center gap-2"
          >
