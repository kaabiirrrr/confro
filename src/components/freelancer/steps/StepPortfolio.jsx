  ...
  return (
    ...
    >
      ...
      <div className="hidden sm:flex justify-between gap-3 sm:gap-5 pt-2 sm:pt-4">
        ...
        >
          {loading ? (
            <><div className="animate-spin rounded-full h-4 w-4 sm:h-5 sm:w-5 border-2 border-primary border-t-transparent mr-2" />Uploading...</>
          ) : "Continue"}
        </button>
      </div>
      ...
      <div className="fixed sm:hidden bottom-0 left-0 right-0 z-40 bg-primary border-t border-white/5 px-4 py-3 flex gap-3">
        ...
        <button onClick={handleNext} disabled={loading} className="flex-1 bg-accent text-white font-bold py-2.5 rounded-full hover:bg-accent/90 disabled:opacity-50 flex items-center justify-center text-sm">
