...
const ResetPassword = () => {
    ...
    useEffect(() => {
        ...
        // user arrives via the reset link, regardless of hash vs PKCE flow
        const { data: { subscription } } = supabase.auth.onAuthStateChange((event) => {
            if (event === 'PASSWORD_RECOVERY') {
                // Session is ready — nothing extra needed, updateUser will work
            }
        });

        // Also try to exchange hash token for older link formats
    ...
    return (
        <div className="min-h-screen bg-primary flex flex-col font-sans selection:bg-accent/30">
            {/* Sticky navbar */}
            <div className="fixed top-0 left-0 w-full h-14 px-6 md:px-14 flex items-center justify-between z-50 bg-primary/80 backdrop-blur-md border-b border-slate-200 dark:border-white/5">
                <Link to="/" aria-label="Connect Home">
                    <img src="/Logo-LightMode-trimmed.png" alt="Connect" className="h-8 object-contain block dark:hidden" />
                    <img src="/Logo2.png" alt="Connect" className="h-7 object-contain hidden dark:block" />
                </Link>
            ...
            {/* True center — offset by half navbar height so it looks visually centered */}
