...
const ProfileMenuClient = () => {
  const { profile, logout, user, setProfile } = useAuth();
    ...
    <div className="absolute top-[calc(100%+12px)] right-0 max-sm:left-0 max-sm:right-0 max-sm:mx-2 sm:w-[300px] p-1 bg-secondary border border-border rounded-2xl shadow-2xl z-50 animate-in fade-in slide-in-from-top-2 duration-200 fixed-mobile-menu">
      <div className="w-full">
        ...
        <div className="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-4 border-b border-border">
          <span className="text-xs sm:text-sm text-light-text">Online for messages</span>
          <div onClick={toggleOnline} className={`w-9 h-5 sm:w-10 sm:h-6 rounded-full relative cursor-pointer transition ${online ? "bg-accent" : "bg-gray-500"}`}>
            <div className={`w-3.5 h-3.5 sm:w-4 sm:h-4 bg-white rounded-full absolute top-[3px] sm:top-1 transition ${online ? "right-1" : "left-1"}`} />
          </div>
        </div>

        <div className="flex flex-col gap-0.5">
          <Link to="/client/account-health" className="dropdown-item !py-2 sm:!py-2.5 !text-xs sm:!text-sm">
          ...
          <Link to="/client/identity-verification" className="dropdown-item !py-2 sm:!py-2.5 !text-xs sm:!text-sm">
            <img src="/Icons/icons8-verified-48.png" alt="" className="w-4 h-4 sm:w-5 sm:h-5 object-contain" style={{ filter: 'var(--dropdown-icon-filter)' }} />
            Identity verification
          </Link>
          <Link to="/client/membership" className="dropdown-item !py-2 sm:!py-2.5 !text-xs sm:!text-sm">
          ...
          <Link to="/client/invite" className="dropdown-item !py-2 sm:!py-2.5 !text-xs sm:!text-sm">
            <img src="/Icons/icons8-add-administrator-100.png" alt="" className="w-4 h-4 sm:w-5 sm:h-5 object-contain" style={{ filter: 'var(--dropdown-icon-filter)' }} />
            Invite a coworker
          </Link>
          <ThemeToggle />
          <Link to="/client/settings" className="dropdown-item !py-2 sm:!py-2.5 !text-xs sm:!text-sm">
        ...
        <div className="mt-1 pt-1 border-t border-border">
          <button onClick={handleLogout} className="dropdown-item w-full text-left !text-red-600 hover:bg-red-500/10 hover:!text-red-700 group/logout transition-colors !py-2 sm:!py-2.5 !text-xs sm:!text-sm">
